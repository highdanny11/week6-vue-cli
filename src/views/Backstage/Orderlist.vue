      ...
      <table class="table" v-if="chang">
        ...
        <tbody>
          <tr v-for="item in orders" :key="item.id">
            ...
            <td>{{ item.id }}</td>
            <td>{{ item.create_at.toLocaleString() }}</td>
      ...
      <table class="table table-danger" v-if="!chang">
        <thead>
          ...
          <tr>
            ...
            <td>{{ order.create_at.toLocaleString() }}</td>
            <td>
        ...
        .then((res) => {
          if (res.data.success) {
            ...
            this.orders.forEach((item) => {
              if (JSON.stringify(item.create_at).length === 10) {
                item.create_at = new Date(
                  JSON.parse(JSON.stringify(item.create_at) + '000')
                )
              } else {
                item.create_at = new Date(
                  JSON.parse(JSON.stringify(item.create_at))
                )
              }
            })
